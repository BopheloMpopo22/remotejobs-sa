...
async function getTopJobsByCategory() {
  try {
    // Define job categories and their search terms for multiple APIs
    ...
    for (const [category, searchTerms] of Object.entries(categoryMappings)) {
      try {
        console.log(
          `🔍 Fetching jobs for ${category} from multiple sources...`
        );
        ...
        if (errors.length > 0) {
          console.log(`⚠️ API errors for ${category}:`, errors);
        }

        console.log(
          `📊 Found ${jobs.length} total jobs for ${category} from sources:`,
          sources
        );
        ...
        const top2Jobs = jobs
          ...
          .map((job) => ({
            ...
            url: job.url,
            source: job.source, // Add source info for transparency
          }));

        topJobs[category] = top2Jobs;
        console.log(`✅ Selected ${top2Jobs.length} top jobs for ${category}`);
      } catch (error) {
  ...
  const jobsHtml = jobCategories
    .map((category) => {
      ...
      const jobsList = jobs
       ...
       💰 ${job.salary} | 📍 ${job.location} | 📊 ${job.source}<br>
